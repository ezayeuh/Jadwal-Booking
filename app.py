...
# CSS Kustom untuk Tombol Kalender yang Rapih & Elegan
st.markdown("""
<style>
    .banner {
    ...
    /* Ukuran dan bentuk tombol kalender agar pas di HP */
    .stButton button {
        width: 100%;
        height: 65px;
        border-radius: 8px;
...
# -------------------------------------------------------------
# RENDER KALENDER GRID TOMBOL STREAMLIT (STABIL & ANTI-ERROR)
# -------------------------------------------------------------
...
for i, h_name in enumerate(hari_names_singkat):
    ...
    with cols_hdr[i]:
        st.markdown(f"<p style='text-align: center; font-weight: bold; color: {c_color}; margin-bottom: 5px; font-size: 13px;'>{h_name}</p>", unsafe_allow_html=True)
    ...
    for day_idx, day in enumerate(week):
        with cols[day_idx]:
            if day == 0:
                ...
                # Teks dalam tombol kotak tanggal
                if jml_ev > 0:
                    label_btn = f"📌 {day}\n\n🔴 {jml_ev} Rombel"
                else:
                    label_btn = f"📅 {day}\n\n⚪ Kosong"
                
                # Aksi klik tombol tanggal (Langsung memperbarui state tanpa reload URL)
